...
public class CameraRig3 : MonoBehaviour, ICameraRig
{
    /// <summary>
    /// The left eye camera.
    /// </summary>
    ...
    /// </summary>
    public event System.Action<CameraRig3> UpdatedAnchors;
    ...
    public List<float> GetCameraHandsRotation()
    {
        ...
        // Debug.Log("---Methoddata---Camerarig3alpha: " + alpha);
        return results;
            ...
            if (hmdPresent)
            {
                        ...
                        out centerEyeRotation))
                {
                    ...
                    if (frameCounter >= 60)
                    {
                        ...
                        // Debug.Log("現実の頭のY軸回転 : " + centerEyeRotation.eulerAngles.y);
                        // Debug.Log("originalyaw: " + originalYaw);
                        ...
                        // Debug.Log("今計算された倍率 : " + Math.Abs(RealHeadRotation.y) / Math.Abs(centerEyeRotation.eulerAngles.y));
                        frameCounter = 0;
                    }
    ...
    protected void CheckForAnchorsInParent()
    {
        void Check<T>(Transform node) where T : MonoBehaviour
        {
            ...
            if (anchor && anchor.enabled)
            {
                ...
                Debug.LogError(
                    $"The {typeof(T).Name} '{anchor.name}' is a parent of the {nameof(CameraRig3)} '{name}', which is not allowed. An {typeof(T).Name} may not be the parent of an {nameof(CameraRig3)} because the {nameof(CameraRig3)} defines the tracking space for the anchor, and its transform is relative to the {nameof(CameraRig3)}.");
            }
        }
