...
public class CameraRig5 : MonoBehaviour
{
    /// <summary>
    /// The left eye camera.
    /// </summary>
    ...
    /// </summary>
    public event System.Action<CameraRig5> UpdatedAnchors;
    ...
    //////////////////////////
    public float alpha = 1.0f;  // 頭の回転増幅用のリマッピングパラメータ
    public float handAlpha = 1.0f;  // 手の回転増幅用のリマッピングパラメータ

    private Vector3 prevRealHeadRotation = new Vector3(-1f, -1f, -1f);  // 前フレームの現実の頭の回転
    private Vector3 prevRealHandRotation = new Vector3(-1f, -1f, -1f);  // 前フレームの現実の手の回転
    private Vector3 prevVirtualHandRotation = new Vector3(-1f, -1f, -1f);  // 前フレームの仮想の手の回転

    private bool initialHandRotation = true;  // 手の初期回転処理
    ...
    protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
    {
        ...
        {
            if (initialHandRotation)
            {
                prevRealHandRotation = leftHandAnchor.localRotation.eulerAngles;
                prevVirtualHandRotation = prevRealHandRotation;
                initialHandRotation = false;
            }
            else
            {
                Vector3 realHandRotation = leftHandAnchor.localRotation.eulerAngles;
                float deltaHandYRotation = Mathf.DeltaAngle(prevRealHandRotation.y, realHandRotation.y);
                float adjustedHandYRotation = deltaHandYRotation * handAlpha;

                leftHandAnchor.localRotation = Quaternion.Euler(
                    realHandRotation.x,
                    prevVirtualHandRotation.y + adjustedHandYRotation,
                    realHandRotation.z
                );

                prevRealHandRotation = realHandRotation;
                prevVirtualHandRotation = leftHandAnchor.localRotation.eulerAngles;

                // 右手の回転処理 (同様のロジックを適用)
                Vector3 realRightHandRotation = rightHandAnchor.localRotation.eulerAngles;
                float deltaRightHandYRotation = Mathf.DeltaAngle(prevRealHandRotation.y, realRightHandRotation.y);
                float adjustedRightHandYRotation = deltaRightHandYRotation * handAlpha;

                rightHandAnchor.localRotation = Quaternion.Euler(
                    realRightHandRotation.x,
                    prevVirtualHandRotation.y + adjustedRightHandYRotation,
                    realRightHandRotation.z
                );
            }
        }
    ...
    protected void CheckForAnchorsInParent()
    {
        void Check<T>(Transform node) where T : MonoBehaviour
        {
            ...
            if (anchor && anchor.enabled)
            {
                ...
                Debug.LogError(
                    $"The {typeof(T).Name} '{anchor.name}' is a parent of the {nameof(CameraRig5)} '{name}', which is not allowed. An {typeof(T).Name} may not be the parent of an {nameof(CameraRig5)} because the {nameof(CameraRig5)} defines the tracking space for the anchor, and its transform is relative to the {nameof(CameraRig5)}.");
            }
        }
