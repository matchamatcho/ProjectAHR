...
public class CameraRig6 : MonoBehaviour, ICameraRig
{
    /// <summary>
    /// The left eye camera.
    /// </summary>
    ...
    /// </summary>
    public event System.Action<CameraRig6> UpdatedAnchors;
    ...
    public List<float> GetCameraHandsRotation()
    {
        ...
        // Debug.Log("---Methoddata---Camerarig6alpha: " + alpha);
        return results;
    ...
    protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
    {
        ...
        if (updateEyeAnchors)
        {
            if (hmdPresent)
            {
                        ...
                        OVRPlugin.Node.EyeCenter, OVRPlugin.Step.Render, out centerEyePosition))
                {
                    ...
                    // centerEyeAnchor.localPosition = new Vector3(0,centerEyePosition.y,0);


                    realHeadPosition = centerEyePosition;
                    // objectToRotate.localPosition.x = centerEyePosition.x;
                    // objectToRotate.localPosition.z = centerEyePosition.z;
                    // objectToRotate.localPosition=new Vector3(centerEyePosition.x,0,centerEyePosition.z);
                        ...
                        out centerEyeRotation))
                {
                    ...
                    if (frameCounter >= 60)
                    {
                        ...
                        // Debug.Log("realheadYrotation: " + originalYaw);
                        // // Debug.Log("現実の頭のY軸回転(制御後) : " + originalYaw);
                        // Debug.Log("vertualheadrotation : " + centerEyeAnchor.eulerAngles.y );
                        // Debug.Log("vertualheadtotation tashizann: " + (originalYaw+amplifiedYaw) );
                        // Debug.Log("realalpha: " + alpha);
                        // Debug.Log("nowcallicurated : " +  (originalYaw+amplifiedYaw)/ (originalYaw));
                        frameCounter = 0;
                    }
        ...
        if (updateHandAnchors)
        {
            ...
            float amplifiedAngleLeft = angleFromFrontLeft + DeltaRotation;
            ...
            float amplifiedAngleRight = angleFromFrontRight + DeltaRotation;
            ...
            float amplifiedYawLeft = originalYawLeft + DeltaRotation;
            ...
            float amplifiedYawRight = originalYawRight + DeltaRotation;
    ...
    protected void CheckForAnchorsInParent()
    {
        void Check<T>(Transform node) where T : MonoBehaviour
        {
            ...
            if (anchor && anchor.enabled)
            {
                ...
                Debug.LogError(
                    $"The {typeof(T).Name} '{anchor.name}' is a parent of the {nameof(CameraRig6)} '{name}', which is not allowed. An {typeof(T).Name} may not be the parent of an {nameof(CameraRig6)} because the {nameof(CameraRig6)} defines the tracking space for the anchor, and its transform is relative to the {nameof(CameraRig6)}.");
            }
        }
