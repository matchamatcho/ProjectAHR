...
public class CameraRig1 : MonoBehaviour, ICameraRig
{
    /// <summary>
    /// The left eye camera.
    /// </summary>
    ...
    /// </summary>
    public event System.Action<CameraRig1> UpdatedAnchors;
    ...
    private Vector3 realHeadPosition = new Vector3(0, 0.25f, 0);
    ...
    public List<float> GetCameraHandsRotation()
    {
        ...
        // Debug.Log("---Methoddata---Camerarig1alpha: ");
    ...
    protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
    {
        ...
        if (updateEyeAnchors)
        {
            if (hmdPresent)
            {
                        ...
                        out centerEyeRotation))
                {
                    ...
                    float amplifiedYaw = originalYaw;
        ...
        if (updateHandAnchors)
        {
            ...
            float amplifiedAngleLeft = angleFromFrontLeft;
            ...
            float amplifiedAngleRight = angleFromFrontRight;
            ...
            float amplifiedYawLeft = originalYawLeft;
            ...
            float amplifiedYawRight = originalYawRight;
        ...
        // Debug.Log($"---everyrp---cr1:{Time.frameCount}, {everyFrameHeadRotation[1]},{everyFrameRightHandRotation[1]},{everyFrameRightHandPosition[0]},{everyFrameRightHandPosition[1]},{everyFrameRightHandPosition[2]}");
    ...
    protected void CheckForAnchorsInParent()
    {
        void Check<T>(Transform node) where T : MonoBehaviour
        {
            ...
            if (anchor && anchor.enabled)
            {
                ...
                Debug.LogError(
                    $"The {typeof(T).Name} '{anchor.name}' is a parent of the {nameof(CameraRig1)} '{name}', which is not allowed. An {typeof(T).Name} may not be the parent of an {nameof(CameraRig1)} because the {nameof(CameraRig1)} defines the tracking space for the anchor, and its transform is relative to the {nameof(CameraRig1)}.");
            }
        }
